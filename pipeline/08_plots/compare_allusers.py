"""
Compare each "all-users" DPO model (one per beta) against the AVERAGE OCEAN
profile of the 100 users.

Model OCEAN  = mean across the 3 evaluation tests (BFI-2, Goldberg-100, FFPI),
               matching how the per-user plots aggregate.
User average = mean over the 100 users of their actual IPIP-300 domain scores,
               put on the 1-5 scale (domain sum / 60), same convention as
               master_users.py.

Writes a comparison table (printed + JSON) and a bar chart with the user
average drawn as a dashed reference line per trait.

Usage: python3 compare_allusers.py
"""
import json
import numpy as np
import pandas as pd
...
TESTS=["BFI-2","FFPI"]   # Goldberg-100 dropped from the eval
...
ids=json.load(open("user_ids_100.json"))
df=pd.read_csv("ipip300_responses_and_scores.csv")
...
# --- model OCEAN per beta (avg across the 3 tests) ---
def model_ocean(beta):
    tbl=json.load(open(f"results_{SHORT}_ALLUSERS_beta{beta}.json"))["table"]
...
# mean absolute deviation from the user average, per beta (how close overall)
print()
for b in BETAS:
    mad=np.mean([abs(models[b][c]-user_avg[c]) for c in CODES])
    print(f"beta {b}: mean |model - user avg| = {mad:.3f}")
json.dump({"user_avg":{c:user_avg[c] for c in CODES},"per_trait":rows}, open("allusers_comparison.json","w"), indent=2)
...
ax.legend(); plt.tight_layout(); plt.savefig("allusers_vs_avg.png",dpi=150); plt.close()
print("\nwrote allusers_comparison.json and allusers_vs_avg.png")
