"""Phase-A plots for WizardLM-70B-V1.0: AGENT_LOW/HIGH (3 bars) + macro_delta_summary."""
import os, json
import numpy as np
import matplotlib; matplotlib.use("Agg"); import matplotlib.pyplot as plt
HERE=os.path.dirname(os.path.abspath(__file__)); SHORT="WizardLM-70B-V1.0"
...
TESTS=["BFI-2","FFPI"]; SERIES=["Baseline (SFT)"]+TESTS; COLORS=["#7f7f7f","#4C72B0","#C44E52"]; BETAS=["0.01","0.1","0.5"]
def load(label):
    p=os.path.join(HERE,f"results20_{SHORT}_{label}.json"); return json.load(open(p))["table"] if os.path.exists(p) else None
base=load("baseline")
...
def macro(label):
    t=load(label); return np.mean([np.mean([t[ti][c][0] for ti in TESTS]) for c in CODES]) if t else None
bm=macro("baseline")
rows=[(f"{d.upper()} β={b}", macro(f"{d}_beta{b}")-bm) for d in ["high","low"] for b in BETAS if macro(f"{d}_beta{b}") is not None]
