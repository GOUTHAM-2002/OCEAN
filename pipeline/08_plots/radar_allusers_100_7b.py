...
WizardLM-13B-V1.2, over the 100 current users. All-users version of
radar_avg_users_100.py.

Instead of averaging 100 per-user DPO'd models, this uses the 3 single models
each DPO-trained on ALL 100 users' combined preferences at one beta:
  results_<short>_ALLUSERS_beta{0.01,0.1,0.5}.json
...
for reference, computed identically to radar_avg_users_100.py.

The radial axis uses a FIXED range shared with avg_users_radar_100.png, computed
from the union of all values appearing in EITHER plot, so the two charts are
directly comparable.

Writes: g_plots/WizardLM/WizardLM-13B-V1.2/allusers_radar_100.png
"""
import os, json
...
def shared_range():
    """Fixed [lo, hi] shared with avg_users_radar_100.png. Unions every value
    that appears in EITHER plot: target + baseline, the per-user-averaged DPO
    lines, and the all-users (pooled) lines. Same padding formula as the
    existing script."""
...
def make_radar(outfile):
    ...
    # filled (not hollow). All-users betas first / underneath, then baseline, then
    # the user target as a bold line on top so the user-vs-model gap is easy to read.
    for b in BETAS:
        if b in allu:
    ...
    # fixed radial range shared with avg_users_radar_100.png (not auto-zoom)
    lo, hi = shared_range()
