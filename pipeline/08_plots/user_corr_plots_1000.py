...
Outputs (one figure per beta, 5 trait subplots each, up to 1000 points each):
  g_plots/WizardLM/WizardLM-13B-V1.2/USERS_1000_CORR/users1000_beta<b>_corr.png
...
SHORT = os.environ.get("SWEEP_SHORT", "WizardLM-13B-V1.2")
FAMILY = os.environ.get("SWEEP_FAMILY", "WizardLM")
USERS_FILE = os.environ.get("USERS_FILE", "user_ids_1000.json")
OUTD = os.path.join(HERE, "g_plots", FAMILY, SHORT, "USERS_1000_CORR")
TESTS = ["BFI-2", "FFPI"]
CODES = ["O", "C", "E", "A", "N"]
...
def user_ids():
    return json.load(open(os.path.join(HERE, USERS_FILE)))
