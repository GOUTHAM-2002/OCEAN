...
todo = [c for c in IDS if not os.path.exists(f"results_worldtour/{c}.json")
...
for g, cs in groups.items():
    ...
    log = open(f"worldtour_logs/eval_gpu{g}.log", "w")
    p = subprocess.Popen(["python3", "-u", "worldtour_eval.py", str(g), ",".join(cs)],
