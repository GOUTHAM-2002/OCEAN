...
BETA = "0.01"
OUT = os.path.join(HERE, "adapters_worldtour"); os.makedirs(OUT, exist_ok=True)
IDS = sorted(int(k) for k in json.load(open("worldtour_meta.json")))
...
while queue or running:
    ...
    while queue and fg:
        ...
        data = os.path.join(HERE, "user_dpo_worldtour", f"{cid}.jsonl")
        outd = os.path.join(OUT, str(cid))
        env = dict(os.environ, CUDA_VISIBLE_DEVICES=str(gpu),
                   HF_HOME="/mnt/ssd3/hf_cache", HF_HUB_OFFLINE="1")
        log = os.path.join(HERE, "worldtour_logs"); os.makedirs(log, exist_ok=True)
...
print("WORLDTOUR_TRAIN_DONE", flush=True)
